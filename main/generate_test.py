...
def generation_pipeline():
    ...
    # target focal method should use the test descriptions generated from focal method
    test_desc_from_fm_data = load_test_desc_from_fm_dataset(setting=args.test_desc_setting)
    assert len(coverage_data) == len(test_desc_from_fm_data)
    for each1, each2, each3 in zip(test_desc_data, test_desc_from_fm_data, coverage_data):
        assert each1['target_test_case'] == each2['test_case'] == each3.test_case, f'Inconsistent test case: {each1["target_test_case"]} vs {each2["test_case"]} vs {each3.test_case}'
    ...
    for target_pair_idx, each_target_pair in tqdm(enumerate(coverage_data), total=len(coverage_data), ncols=80, desc='Generating test cases'):
        ...
        # assert test_desc_from_fm_data[target_pair_idx]['focal_method'] == target_focal_method
        if test_desc_from_fm_data[target_pair_idx]['matching_level'] != 'High':
            print(f'[INFO] The matching level is {test_desc_from_fm_data[target_pair_idx]["matching_level"]}, skip this sample.\n')
            continue
        target_test_case_desc = test_desc_from_fm_data[target_pair_idx]['test_desc']['under_setting']
        assert (args.test_desc_setting == 'none' and target_test_case_desc.strip() == '') or (args.test_desc_setting != 'none' and target_test_case_desc.strip() != ''), f'Inconsistent test description setting for {target_pair_idx}: {args.test_desc_setting} vs "{target_test_case_desc}"'
...
def load_test_desc_from_fm_dataset(setting):
    with open(configs.matched_test_desc_with_tc_dataset_path, 'r') as f:
        test_desc_from_fm_data = json.load(f)

    test_desc_data_reformat = []
    for each in test_desc_from_fm_data:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--junit_version', type=str, required=True)
