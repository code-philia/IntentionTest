...
class StatisticKG():
    def __init__(self, test_case_log_and_coverage_save_path, specified_target_coverage_ids: list=None):
        ...
        self.test_case_log_analysis = self.load_test_case_coverage(test_case_log_and_coverage_save_path)

    def load_test_case_coverage(self, test_case_log_and_coverage_save_path):
        with open(test_case_log_and_coverage_save_path, 'r') as f:
            test_case_log_and_cov = json.load(f)
        fail_compile, fail_execute, fail_pass, success = 0, 0, 0, 0
        for each_tc in test_case_log_and_cov:
            if each_tc['running_result'] == 'fail_compile':
                fail_compile += 1
            elif each_tc['running_result'] == 'fail_execute':
                fail_execute += 1
            elif each_tc['running_result'] == 'fail_pass':
                fail_pass += 1
            elif each_tc['running_result'] == 'success':
                success += 1

        print(f'Fail Compile: {fail_compile}, Fail Execute: {fail_execute}, Fail Pass: {fail_pass}, Success Pass: {success}')

        return test_case_log_and_cov
    ...
    def count_test_case_pass(self, is_ref):
        success_pass, fail_compile, fail_execute, fail_pass, unknown = self._count_test_case_pass(is_ref)
        print(f'[{is_ref}]\nFail Compile: {len(fail_compile)}, Fail Execute: {len(fail_execute)}, Fail Pass: {len(fail_pass)}, Unknown: {len(unknown)}, Success Pass: {len(success_pass)}\n')
    ...
    def _count_test_case_pass(self, is_ref):
        ...
        for each_tc_log_cov in self.test_case_log_analysis:
            referable = "referable" if is_ref == "rag_ref" else "not_referable"
            actual_referable = "referable" if len(each_tc_log_cov['rag_references'][0][-1])>0 else "not_referable"
            if actual_referable != referable:
                continue
            result_type = each_tc_log_cov[f'result_rag_ref']
    ...
    def cal_codebleu_for_test_cases(self, is_pass: bool=False, is_common: bool=False):
        test_case_rag_ref_target_pairs = self.load_test_cases(is_pass=is_pass)
        # test_case_rag_ref_target_pairs = self.test_case_log_analysis
    ...
    def load_test_cases(self, is_pass: bool=False, return_focal_method=False, is_ref = "all"):
        ...
        for each_test_case in self.test_case_log_analysis:
            ...
            if (not is_pass) or each_test_case['running_result'] == 'success':
